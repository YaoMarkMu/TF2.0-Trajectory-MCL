...
class SlimHumanoidEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def tf_reward_fn(self):
        def _thunk(obs, act, next_obs):
            ...
            lin_vel_cost = 0.25 / 0.015 * obs[..., 22]
            quad_ctrl_cost = 0.1 * tf.compat.v1.compat.v1.reduce_sum(tf.compat.v1.compat.v1.square(ctrl), axis=-1)
            quad_impact_cost = 0.0

            alive_bonus = 5.0 * tf.compat.v1.compat.v1.cast(
                tf.compat.v1.compat.v1.logical_and(tf.compat.v1.compat.v1.greater(obs[..., 1], 1.0), tf.compat.v1.compat.v1.less(obs[..., 1], 2.0)),
                dtype=tf.compat.v1.compat.v1.float32,
            )
