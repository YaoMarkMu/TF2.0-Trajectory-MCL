...
class HalfCheetahEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def obs_preproc(self, obs):
        if isinstance(obs, np.ndarray):
            ...
            return tf.compat.v1.compat.v1.concat(
                [
                    obs[..., 1:2],
                    tf.compat.v1.compat.v1.sin(obs[..., 2:3]),
                    tf.compat.v1.compat.v1.cos(obs[..., 2:3]),
                    obs[..., 3:],
                ],
                axis=-1,
            )
    ...
    def obs_postproc(self, obs, pred):
        if isinstance(obs, np.ndarray):
            ...
            return tf.compat.v1.compat.v1.concat([pred[..., :1], obs[..., 1:] + pred[..., 1:]], axis=-1)
    ...
    def tf_reward_fn(self):
        def _thunk(obs, act, next_obs):
            ctrl_cost = 1e-1 * tf.compat.v1.compat.v1.reduce_sum(tf.compat.v1.compat.v1.square(act), axis=-1)
            forward_reward = obs[..., 0]
            reward = forward_reward - ctrl_cost
            return reward
